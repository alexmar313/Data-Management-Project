...
# ADDITIONS 
to make a better graph we should show the numbers as a percentage of total rather than just numbers. so, suspected non-citizens as a % of total non-citizens, and compare that to suspected citizens as a % of total citizens, etc...
to do that, we can use the following dataset:
...
pop_data_non_citizens <- pop_data %>%
...
effect of polarizqatino on total conviction.
```{r}
