...
manifesto2 <- manifesto %>%
  select(countryname, edate, partyname, per101, per102, per103, per104, per105, per106, per107, per108, per109, per201, per203, per202, per204, per301, per302, per303, per305)

manifesto_long <- manifesto2 %>%
  pivot_longer(
    cols = starts_with("per"),  # Select all `per` variables
    names_to = "per_category",  # Create a column for the `per` variable names
    values_to = "value"         # Create a column for the `per` values
  )

manifesto_long <- manifesto_long %>%
  mutate(party_per = paste0(partyname, "/", per_category))  # Combine party and per category

# Keep relevant columns
manifesto_wide <- manifesto_long %>%
  select(countryname, edate, party_per, value) 

manifesto_wide <- manifesto_wide %>%
  pivot_wider(
    names_from = party_per,  # Each unique `party_per` becomes a column
    values_from = value      # Populate with values from the `value` column
  )

manifesto_wide <- manifesto_wide %>%
  drop_na(value)

unique_columns <- manifesto_long %>%
  distinct(party_per) %>%
  nrow()
...
manifesto_summary <- get_summary(manifesto, "Manifesto Dataset", "edate", "year-month-day", "countryname")
